...
TEST_F(correctness_test, iterator_compare_3) {
  container c;
  mass_push_back(c, {3, 4});
  mass_push_front(c, {2, 1});
  expect_eq(c, {1, 2, 3, 4});

  EXPECT_LT(c.begin(), c.end());

  EXPECT_LE(c.begin(), c.begin() + 0);
  EXPECT_LE(c.begin(), c.begin() + 1);
  EXPECT_LE(c.begin(), c.begin() + 2);
  EXPECT_LE(c.begin(), c.begin() + 3);

  EXPECT_LE(c.begin() + 1, c.end());
  EXPECT_LE(c.begin() + 2, c.end());
  EXPECT_LE(c.begin() + 3, c.end());
  EXPECT_LE(c.begin() + 4, c.end());
}
